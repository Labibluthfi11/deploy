...
<div class="mb-6">
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            {{-- Search & Export (Contoh) --}}
            <div class="relative">
                <input type="text" placeholder="Cari karyawan..."
                    class="pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                <svg class="absolute left-3 top-2.5 h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                </svg>
            </div>
            <button class="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors shadow-sm">
                Export Data
            </button>
        </div>
    </div>
...
<div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                {{-- Filters (Contoh) --}}
                <select class="px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($submissions as $index => $submission)
                    @php
                        ...
                        // --- LOGIKA DURASI LEMBUR (PERUBAHAN: Potongan Istirahat 30 Menit) ---
                        $duration = '-';
                        ...
                        if ($isOvertime && $submission->lembur_start && $submission->lembur_end) {
                            ...
                            // Potong 30 menit (0.5 jam) jika lembur_rest = 1
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="text-sm text-gray-900 max-w-xs">
                                ...
                                {{-- 🆕 TAMPILKAN HISTORY REJECTION (Jika pernah ditolak & sekarang pending lagi) --}}
                                        ...
                                        <div class="flex items-start gap-2">
                                            ...
                                            <div class="flex-1">
                                                <p class="font-bold text-amber-800 mb-1">
                                                    ⚠️ Pernah Ditolak oleh {{ strtoupper(str_replace(['_', '-'], ' ', $submission->rejected_by)) }}
                                                </p>
                                                <p class="text-amber-700 italic leading-tight">
                                                    "{{ $submission->catatan_admin }}"
                                                </p>
                                                @if($submission->rejected_at)
                                                    <p class="text-amber-600 mt-1.5 text-xs">
                                                        📅 {{ \Carbon\Carbon::parse($submission->rejected_at)->isoFormat('DD MMM YYYY, HH:mm') }}
                                                        <span class="text-amber-500">({{ \Carbon\Carbon::parse($submission->rejected_at)->diffForHumans() }})</span>
                                                    </p>
                                                @endif
                                            </div>
                                        </div>
                                ...
                                {{-- Status rejection AKTIF (kalau baru ditolak & belum resubmit) --}}
                                ...
                                @if ($submission->tipe !== 'lembur')
                                    <p class="text-xs text-gray-500 mt-1.5">
                                        {{ $submission->status ? 'Pengajuan ' . ucfirst($submission->status) : 'Absensi Reguler' }}
                                    </p>
                                @else
                                    <p class="text-xs text-gray-500 mt-1.5">
                                        Diajukan {{ $submission->created_at ? \Carbon\Carbon::parse($submission->created_at)->diffForHumans() : '-' }}
                                    </p>
                                @endif
                            </div>
                        </td>
