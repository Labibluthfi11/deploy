    ...
    private function indexByEmploymentType(Request $request, ?string $type)
    {
            ...
            $comparison = [
                'organik' => [
                    'hadir' => Absensi::whereHas('user', fn($q) => $q->where('employment_type', 'organik'))
                        ->where('status_approval', 'approved')->where('status', 'hadir')
                        ->whereYear('check_in_at', $year)->whereMonth('check_in_at', $month)->count(),
                    'izin' => Absensi::whereHas('user', fn($q) => $q->where('employment_type', 'organik'))
                        ->where('status_approval', 'approved')->where('status', 'izin')
                        ->whereYear('check_in_at', $year)->whereMonth('check_in_at', $month)->count(),
                    'sakit' => Absensi::whereHas('user', fn($q) => $q->where('employment_type', 'organik'))
                        ->where('status_approval', 'approved')->where('status', 'sakit')
                        ->whereYear('check_in_at', $year)->whereMonth('check_in_at', $month)->count(),
                    'lembur' => Absensi::whereHas('user', fn($q) => $q->where('employment_type', 'organik'))
                        ->where('status_approval', 'approved')->where('tipe', 'lembur')
                        ->whereYear('check_in_at', $year)->whereMonth('check_in_at', $month)->count(),
                ],
                'freelance' => [
        ...
        for ($m = 1; $m <= 12; $m++) {
            $grafikBulananOrganik[$m] = Absensi::whereHas('user', fn($q) => $q->where('employment_type', 'organik'))
                ->where('status_approval', 'approved')->whereYear('check_in_at', $year)->whereMonth('check_in_at', $m)->count();
        ...
        $grafikBulanan = [];
        for ($m = 1; $m <= 12; $m++) {
            $grafikBulanan[$m] = Absensi::whereYear('check_in_at', $year)->whereMonth('check_in_at', $m)
                ->where('status_approval', 'approved')->where($absensiQueryFilter)->count();
        }
        ...
        return view('admin.absensi.index', compact(
            'users', 'month', 'year', 'grafikBulanan', 'grafikBulananOrganik', 'grafikBulananFreelance',
            'pendingApprovals', 'dailyStatuses', 'dailyStatusesOrganik', 'dailyStatusesFreelance',
            'dailyStatusesBorongan', 'dailyStatusesMagang', 'totalHadir', 'totalIzin', 'totalSakit',
            'totalLembur', 'dashboardTitle', 'comparison'
        ))->with('currentStatus', $type ?? 'semua');
    }

    // ✅ UPDATED: Ada cek organik
    public function show(Request $request, User $user)
    {
        // 🔒 Blokir admin biasa dari data organik
        $kategori = $this->detectKategori($user);
        if ($kategori === 'organik' && !$this->isSuperAdmin()) {
            abort(403, 'Anda tidak memiliki akses ke data karyawan organik.');
        }
        ...
        if ($filterType === 'yearly') {
            ...
            $query->whereYear('check_in_at', $year)->whereMonth('check_in_at', $month);
       ...
       $isOrganikOrMagang = in_array($kategori, ['organik', 'magang']);

return view('admin.absensi.user', compact(
    'user', 'absensi', 'absensiStats', 'weeklySummary', 'isOrganikOrMagang'
));
    }

    // ✅ UPDATED: Ada filter organik
    public function recap(Request $request)
    {
        ...
        foreach ($users as $user) {
            // 🔒 Skip organik kalau bukan super_admin
            $kategori = $this->detectKategori($user);
            if ($kategori === 'organik' && !$this->isSuperAdmin()) {
                continue;
            }
        ...
        return view('admin.absensi.recap', compact(
            'recapData', 'month', 'year', 'range', 'week', 'startDate', 'endDate'
        ))->with('selectedMonth', $month)->with('selectedYear', $year);
    }
    ...
    public function exportUser(Request $request, $id)
    {
        ...
        if ($request->filter_type === 'monthly') {
            $query->whereMonth('check_in_at', $request->month)->whereYear('check_in_at', $request->year);
        } elseif ($request->filter_type === 'weekly') {
            $query->whereYear('check_in_at', $request->year)->whereMonth('check_in_at', $request->month)->where('week_number', $request->week);
    ...
    public function exportSlipGaji(Request $request, User $user)
    {
        ...
        return Excel::download(new SlipGajiExport($user, $absensiStats, $periodeLabel), $fileName);
    }
    ...
    public function bulkExportDetail(Request $request)
    {
        ...
        return Excel::download(new BulkDetailExport($userIds, $startDate, $endDate), $fileName);
    }
    ...
    public function bulkExportPdf(Request $request)
    {
        ...
        foreach ($userIds as $userId) {
            ...
            $data = [
                ...
                'totalGajiPokok'   => $approvedAbsensi->sum('base_salary'),
                'totalPotongan'    => $approvedAbsensi->sum('late_penalty'),
                'potongan'         => $approvedAbsensi->sum('late_penalty'),
                ...
                'terbilang'        => ucwords($terbilangString),
                'stats'            => [
                    'total_hadir'        => $approvedAbsensi->where('status', 'hadir')->count(),
                    'total_gaji_pokok'   => $approvedAbsensi->sum('base_salary'),
                    'total_potongan'     => $approvedAbsensi->sum('late_penalty'),
            ...
            try {
                ...
                $zip->addFromString("Slip_Gaji_{$cleanName}.pdf", $content);
            } catch (\Exception $e) {
    ...
    public function bulkExportSimple(Request $request)
    {
        ...
        try {
            return Excel::download(new BulkSimpleExport($userIds, $startDate, $endDate), $fileName);
        } catch (\Exception $e) {
    ...
    {
        \Log::info('updateCheckIn called', ['id' => $id, 'request' => $request->all()]);

        $request->validate(['new_check_in' => 'required|date_format:Y-m-d\TH:i']);
        ...
        \DB::table('absensis')->where('id', $absensi->id)->update([
        ...
        \Log::info('✅ Check-in berhasil diubah!', ['absensi_id' => $absensi->id, 'new_check_in' => $newCheckIn]);
