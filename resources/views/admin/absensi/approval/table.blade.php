...
<div class="bg-gray-800 rounded-lg border border-gray-700 shadow-lg overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-gray-700 bg-gray-800/50">
        <div class="flex items-center justify-between">
            <div class="text-sm text-gray-400">
                Menampilkan <span class="font-medium text-white">{{ $submissions->count() }}</span> data
            </div>
        </div>
    </div>
            ...
            <thead class="bg-gray-800/50">
                <tr>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-300 uppercase tracking-wider">
                        <input type="checkbox" class="rounded border-gray-600 bg-gray-700 text-indigo-500 focus:ring-indigo-500">
                    </th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-300 uppercase tracking-wider">Karyawan</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-300 uppercase tracking-wider">Jenis / Tanggal</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-300 uppercase tracking-wider">Durasi</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-300 uppercase tracking-wider">Bukti</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-300 uppercase tracking-wider">Keterangan</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-300 uppercase tracking-wider">Status</th>
                    <th scope="col" class="px-6 py-3 text-center text-xs font-semibold text-gray-300 uppercase tracking-wider">Aksi</th>
                </tr>
            </thead>
            ...
            <tbody class="bg-gray-800 divide-y divide-gray-700">
                @forelse ($submissions as $index => $submission)
                    ...
                    <tr class="hover:bg-gray-700/50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    ...
                                    <div class="text-xs text-gray-400">{{ $user->employee_id ?? 'N/A' }}</div>
                                </div>
                            </div>
                        </td>
                        ...
                        {{-- JENIS / TANGGAL & WAKTU --}}
                        <td class="px-6 py-4">
                            <div class="text-xs font-semibold text-indigo-400 uppercase">
                                {{ $submission->tipe ? ucfirst($submission->tipe) : (ucfirst($submission->status) ?? 'Absensi') }}
                            </div>
                            <div class="text-sm text-white font-medium mt-1">
                            ...
                            @if ($submission->tipe === 'sakit' || $submission->tipe === 'izin')
                                @if ($submission->file_bukti)
                                    <a href="{{ asset('storage/' . $submission->file_bukti) }}" target="_blank"
                                       class="inline-flex items-center gap-2 px-3 py-1.5 bg-gray-700 text-gray-300 rounded-lg text-xs font-medium hover:bg-gray-600 transition-all">
                                ...
                                @else
                                    <span class="text-xs text-gray-500 italic">Tidak ada</span>
                                @endif
                            @else
                                <span class="text-sm text-gray-500">-</span>
                            @endif
                        ...
                        <td class="px-6 py-4">
                            <div class="text-sm text-gray-300 max-w-xs">
                                ...
                                <p class="truncate font-medium text-white" title="{{ $mainKeterangan }}">
                                    {{ $mainKeterangan }}
                                </p>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    <form action="{{ route('admin.absensi.approval.action', ['absensi' => $submission->id, 'action' => 'approve']) }}"
                                        ...
                                        <button type="submit"
                                            ...
                                            class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-lg text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-green-500 transition-all shadow-sm hover:shadow">
                                    ...
                                    <button type="button"
                                        ...
                                        class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-lg text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-red-500 transition-all shadow-sm hover:shadow">
                                        <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @elseif (in_array($statusApproval, ['approved_supervisor', 'approved_manager']))
                                <span class="inline-flex items-center text-xs text-gray-400">
                                    <svg class="animate-spin h-3.5 w-3.5 mr-1.5 text-gray-500" fill="none" viewBox="0 0 24 24">
                            ...
                            @elseif ($statusApproval === 'approved_hrga')
                                <span class="inline-flex items-center text-xs text-green-400 font-medium">
                            ...
                            @elseif ($statusApproval === 'rejected')
                                <button type="button" class="inline-flex items-center text-xs text-red-400 font-medium hover:text-red-300">
                                    <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-6 py-12 text-center">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-300">Tidak ada data</h3>
    ...
    <div class="px-6 py-4 border-t border-gray-700 bg-gray-800/50">
        <div class="flex items-center justify-between">
            <div class="text-sm text-gray-400">
                Showing <span class="font-medium text-white">1</span> to <span class="font-medium text-white">{{ $submissions->count() }}</span> of <span class="font-medium text-white">{{ $submissions->count() }}</span> results
            </div>
            <div class="flex items-center gap-2">
                <button class="px-3 py-1.5 border border-gray-600 rounded-lg text-sm font-medium text-gray-400 bg-gray-700 hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed" disabled>
                    Previous
                </button>
                <button class="px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700">
                    1
                </button>
                <button class="px-3 py-1.5 border border-gray-600 rounded-lg text-sm font-medium text-gray-400 bg-gray-700 hover:bg-gray-600">
                    2
                </button>
                <button class="px-3 py-1.5 border border-gray-600 rounded-lg text-sm font-medium text-gray-400 bg-gray-700 hover:bg-gray-600">
                    Next
